...
if __name__ == "__main__":
    ...
    filename_list = [
        r"data/sound_files/male_speech.wav",
    ...
    # encode signals
    for file, th, ph in zip(
        filename_list[start_idx : end_idx + 1],
        th_list[start_idx : end_idx + 1],
        ph_list[start_idx : end_idx + 1],
    ):
        anm_t, s, fs, y = encode_signal(
            file,
            sh_order_input,
            th=math.radians(th),
            ph=math.radians(ph),
    ...
    for i in (fs_list != smallest_fs).nonzero()[0]:
        ...
        ), f"Error in fs for file {filename_list[i]} (min is {smallest_fs} and it has {fs_list[i]})"
    ...
    plot_on_2D(
        ...
        title=f"Encoded Signal N={sh_order_input}\n$\\theta$ = {th_list[start_idx:end_idx+1]} \n$\\phi$ = {ph_list[start_idx:end_idx+1]}",
    )

    # divide to subbands
    DS = 2
    if num_bins > 1:
        anm_t_subbands = divide_anm_t_to_sub_bands(
            anm_t, fs, num_bins=num_bins - 2, low_filter_center_freq=1, DS=DS
        )
    else:
        anm_t_subbands = np.expand_dims(anm_t, axis=0)[:, ::DS, :]

    # divide to time windows
    anm_t_subbands_windowed = divide_anm_t_to_time_windows(
        anm_t_subbands, window_length=tau
    )  # [Window, band pass k ,time samples,SH coeff]
    num_windows = min(anm_t_subbands_windowed.shape[0], max_num_windows)

    Y_p_tag = create_sh_matrix(
        upscale_order, zen=P_th, azi=P_ph, type=SH_type
    )  # upscaling matrix
    target_theta, target_phi = np.array([math.radians(90), math.radians(90)]), np.array(
        [math.radians(45), math.radians(0)]
    )
    mask = np.logical_or.reduce(
        np.sqrt((P_th[:, None] - target_theta) ** 2 + (P_ph[:, None] - target_phi) ** 2)
        < math.radians(5),
        axis=1,
    )
    if mask is not None:
        ...
        plot_on_2D(
            ...
            zen=P_th,
            values=dummy,
            title=f"Spatial Mask {i//tau}\n$\\theta$ = {th_list[start_idx:end_idx+1]} \n$\\phi$ = {ph_list[start_idx:end_idx+1]}",
        )
        # plt.show()

    s_subbands = np.zeros(
        (num_windows, num_bins, P, tau)
    )  # num_windows, num_bins | P = num of SH coeff | tau = num of samples
    ...
    if not (multi_processing):
        ...
        for band in range(num_bins):
            ...
            inner_bar = tqdm(
                total=num_windows, desc="Windows", position=1, leave=False
        ...
        args = [
            (anm_t_subbands_windowed[window, band, :, :].T, mask, None)
            for window in range(num_windows)
            for band in range(num_bins)
        ]
    ...
    file = os.path.join(
        "data/output",
        f"{'_'.join([os.path.basename(file_name).split('.wav')[0] for file_name in filename_list[start_idx:end_idx+1]])}_FBbins_{num_bins}_grid_{grid_type}_th_{'_'.join([str(th) for th in th_list[start_idx:end_idx+1]])}_ph_{'_'.join([str(ph) for ph in ph_list[start_idx:end_idx+1]])}_N_input_{sh_order_input}_{suffix}",
    )
    ...
    if True:
        ...
        for i in range(min(0, 1) * tau, min(num_windows, 1) * tau, tau):
            ...
            plot_on_2D(
                ...
                title=f"Signal in Plane Wave Dictonary Window {i//tau}\n$\\theta$ = {th_list[start_idx:end_idx+1]} \n$\\phi$ = {ph_list[start_idx:end_idx+1]}",
            )
            P_index = np.argmin(
                (P_ph - math.radians(ph_list[0])) ** 2
                + (P_th - math.radians(th_list[0])) ** 2
            )
            ideal_constraint_loss.append(
                np.sum(
                    (Y_p[:, P_index] * s[::DS][i].reshape(-1, 1) - anm_t[::DS][i]) ** 2
                )
            )
            algo_constraint_loss.append(
                np.sum(
                    (Y_p @ s_dict[:, i].reshape(-1, 1) - anm_t[::DS][i].reshape(-1, 1))
                    ** 2
                )
            )
