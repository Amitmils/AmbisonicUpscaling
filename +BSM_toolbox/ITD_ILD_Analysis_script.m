...
%             bin_sig_ref_curr = squeeze(REF_sig(:, p, :, h));
            ...
            if COMPUTE_ITD
                ...
                % ITD with video
                %
                [~, ITD_ref(p, h, m) ] = BSM_toolbox.computeITD_bgu_OrVersion( bin_sig_ref_curr(:, 1),...
                    bin_sig_ref_curr(:, 2), desired_fs, methodITD, high_freq_ITD, 0, ph_DOA(p), fig_for_ITD, saveITDvid, ITDvid_file);
                [~, ITD_BSM_cmplx(p, h, m) ] = BSM_toolbox.computeITD_bgu_OrVersion( bin_sig_BSM_cmplx_curr(:, 1),...
                    bin_sig_BSM_cmplx_curr(:, 2), desired_fs, methodITD, high_freq_ITD, 0, ph_DOA(p), fig_for_ITD, saveITDvid, ITDvid_file);
                [~, ITD_BSM_mag(p, h, m) ] = BSM_toolbox.computeITD_bgu_OrVersion( bin_sig_BSM_mag_curr(:, 1),...
