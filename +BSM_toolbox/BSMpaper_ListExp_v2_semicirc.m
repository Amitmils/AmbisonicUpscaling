...
use_mcroomsim = true;
sig_idx = 1;                                           % 1: male speech, 2,3: female speech
...
%% ================= ATFs preprocessing
% Convert V_k to time domain
V_t = ifft(V_k, filt_samp, 3, "symmetric");
V_t = circshift(V_t, round((size(V_t, 3) - 1) / 2), 3);        
...
Y_a = shmat(N_PW, [th_BSMgrid_vec, ph_BSMgrid_vec]);
a_t = (Y_a * anm_t.').';
a_t = real(a_t);

% Convolove a_t with V_t
V_t_perm = permute(V_t, [3, 1, 2]);

% zero-pad a_t recording to correct length
a_t = [a_t; zeros(filt_samp - 1, Q)];
p_array_t = zeros(size(a_t, 1), n_mic);
for m=1:n_mic
    p_array_t(:, m) = sum(fftfilt(squeeze(V_t_perm(:, m, :)), a_t), 2);
end
...
%% ================= calculate array measurements - freq domain
%{
% Convert anm_t to space domain with ATF directions
Y_a = shmat(N_ATF, [th_array, ph_array]);
a_t = (Y_a * anm_t.').';
