...
%     phi(phi>pi) =  phi(phi>pi) - 2*pi;
    ...
    h_trans_l = [h_trans_l,flipud(conj(h_trans_l(:,2:end-1)))];                         %calc negative freq
    h_trans_r = [h_trans_r,flipud(conj(h_trans_r(:,2:end-1)))];                         %calc negative freq
    
    hobj_pc.data = zeros([size(h_trans_l,1) size(h_trans_l,2) 2]);
